...
function table {  #функция для обработки файлов в директории
 for file in "$1"/*  #цикл для перебора файлов
  ...
  then
   table "$file" #рекурсивный вызов функции для обработки файлов во вложенной директории
  else
   ...
   echo -e "$name \t $extension \t $data_of_change \t $size "kB"  \t $filepath \t $duration_sec"  >> result.xls #дозапись строки вывода в файл .xls
   fi
  done
}
